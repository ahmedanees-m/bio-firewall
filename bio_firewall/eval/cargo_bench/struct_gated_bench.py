"""WS-STRUCT-GATED (v0.8.0) - close the v0.6 struct negative. The v0.6 MEAN ensemble diluted ESM at 1%-FPR
(incremental TPR -0.48) because it averaged in a weak, low-confidence fold channel everywhere. The fix (the
pre-registered next step): a CONFIDENCE-GATED fusion - use the structure signal only where the predicted-structure
...
def run(out_dir: str = "bf_struct_gated", bf_b2: str | None = None, struct_scores: str | None = None,
    ...
    mean_ens = ensemble_score(p_esm, struct)                          # the v0.6 mean ensemble (for contrast)
    ...
    incr_mean = paired_boot_diff(yte, mean_ens, p_esm, cte, tpr_at_fpr)       # v0.6 mean - ESM (the failure)
