"""Benchmark 4b: conformal evaluation. Builds the calibration corpus from the SAME independent
...
def run(out_dir: str = "bf_conformal", seed: int = SEED) -> dict:
    ...
    # the prior (inverted) tiers, recomputed on the same test split for the before/after story
    def old_conf(r):  # old rule: clean allow OR deterministic -> high; mechanism -> moderate; ml -> low
