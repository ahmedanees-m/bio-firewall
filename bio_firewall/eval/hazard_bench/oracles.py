...
definition of hazard - not that it recognized its own gene list (the initial demo's tautology).
...
def cosmic_fusion_pairs() -> frozenset[tuple[str, str]]:
    ...
    Sorted gene-pair tuples; local-only (COSMIC), never committed. The de-novo fusion-mechanism generalization label."""
    p = _oracle_dir() / "cosmic_cgc_v104.tsv"
    if not p.exists():
        return frozenset()
