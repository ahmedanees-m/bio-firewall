"""NP-conformal head-to-head on the firewall corpus. Builds a feature vector per plan from the live
verdict (per-axis severity + competence band + gnomAD pLI + the scalar risk), splits gene-disjointly into
train / calibration / test, and compares two escalate / route-to-review selectors at matched alpha:

  baseline:        threshold the SCALAR risk score, conformal-calibrated on the benign null (P(escalate|benign)<=alpha).
  NP:              a likelihood-ratio statistic over the FULL feature vector + conformal selection on the same null.
...
def _features(plan: dict) -> tuple[list[float], float]:
    """Feature vector + the scalar risk for one plan. Features the scalar collapses: per-axis severity, the
...
def run(out_dir: str = "bf_conformal_np", seed: int = SEED) -> dict:
    ...
    gate = {
        ...
        "fallback_if_fail": "keep the Clopper-Pearson certified bound (already passing); report the null.",
    }
