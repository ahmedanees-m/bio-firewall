"""WS-VERIFY-RECONCILE (v0.9.1) - reconcile PEN-STACK's in-design verdict with the BioFirewall five-axis verdict.

The adapter long promised to "call pen_stack.verify() and reconcile legality + the firewall verdict." Verified against
pen-stack 7.0.0: there is NO top-level pen_stack.verify()/Verdict; the in-design verdict is
`pen_stack.safety.safety_gate(design) -> SafetyVerdict` (decision in {clear, flag, refuse, escalate}). This realizes
the reconciliation against that real API.
