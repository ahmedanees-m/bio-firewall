"""WS-CLOUDLAB-GATE (v0.9.1) - gate the PEN-STACK 7.0 Stage J cloud-lab execution bridge.

PEN-STACK 7.0 added a design->physical-execution path: `pen_stack.build.cloudlab.submit(design, experiment, ...)`
(raises `ProtocolExportError` when its own in-design gate blocks) and `submit_gated(...)` (returns a structured
refusal). PEN-STACK's own gate is the necessary-not-sufficient in-design screen; BioFirewall is Stage K, the
