"""Sub-gene POSITIONAL resolution for the locus axis. A gene-membership lookup asks 'is
...
falls in the promoter / upstream-regulatory window of an oncogene TSS (GENCODE coords x CancerMine CC0 roles).

NOTE: this adds positional FEATURES + mechanism-flagging. The outcome-AUROC-improvement claim from the
locus-outcome analysis is DEFERRED: it depends on controlled-access integration-site outcome data (the open VISDB
floor was the wrong virus biology). So this flags on mechanism, not a validated
...
_SRC = {"source": "GENCODE TSS x CancerMine (CC0) oncogene/driver roles; genotoxic-CIS literature list"}
