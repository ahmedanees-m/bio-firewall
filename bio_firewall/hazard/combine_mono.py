"""A PROVABLY-MONOTONE, interaction-aware evidence combiner that replaces the
...
but can never auto-refuse legitimate research. Decisions are therefore IDENTICAL to the prior cascade
...
def decision_matches_cascade(findings) -> bool:
    """Equivalence guard: the monotone combiner's DECISION must equal the max-severity cascade's decision."""
