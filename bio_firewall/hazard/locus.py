...
oncogene/TSG/driver + DepMap essential, derived from PEN-STACK). Flags: oncogene-activation-by-design
(hard_reject), genotoxic-CIS proximity, tumor-suppressor disruption, essential-gene disruption, oncogene proximity.

NOTE: this axis FLAGS on mechanism; the genotoxicity proxy is NOT outcome-validated (PEN-STACK). It
...
def screen_locus(plan: dict) -> Finding:
    ...
    # gene-membership is clear, but the insertion COORDINATE may sit in an oncogene's promoter/enhancer window
    # (the SCID-X1/LMO2 mechanism a gene-body lookup misses).
