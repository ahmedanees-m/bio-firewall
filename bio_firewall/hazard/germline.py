...
def screen_germline(plan: dict) -> Finding:
    raw = plan.get("germline")
    # A caller may pass a bare boolean (or another non-dict) where the documented schema is a dict; coerce it so a
    # schema mistake in someone else's plan degrades to a screen result rather than a stack trace.
    g = raw if isinstance(raw, dict) else ({"heritable": True} if raw else {})
    loc = plan.get("locus")
    loc = loc if isinstance(loc, dict) else {}
    ct = _norm(g.get("cell_type") or loc.get("cell_type"))
