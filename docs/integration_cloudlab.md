# Gating the PEN-STACK Stage J cloud-lab execution bridge

PEN-STACK 7.0 added a design->physical-execution path (Stage J): `pen_stack.build.cloudlab.submit(design, experiment,
...
BioFirewall is **Stage K**, the comprehensive downstream screen that must run first, in-workflow, where an agent
