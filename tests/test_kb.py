"""The signed hazard KB verifies, every entry carries provenance, and the KB is a SUPERSET
of the in-code signatures (it cannot silently drift from what the screen actually uses)."""
from __future__ import annotations

from bio_firewall.hazard.cargo import SELECT_AGENT_TOXIN_ORGANISMS, SELECT_AGENT_TOXINS
...
def test_kb_is_superset_of_in_code_signatures():
    ...
    tox = {e["value"] for e in entries(_KB, "select_agent_toxin")}
    org = {e["value"] for e in entries(_KB, "select_agent_toxin_organism")}
