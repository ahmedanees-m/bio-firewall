...
from bio_firewall.hazard.cargo import SELECT_AGENT_TOXIN_ORGANISMS, SELECT_AGENT_TOXINS  # noqa: E402
...
def build(kb_version: str, released: str) -> dict:
    ...
    for t in sorted(SELECT_AGENT_TOXINS):
        entries.append({"id": "SATOXIN-" + t.replace(" ", "-"), "type": "select_agent_toxin", "value": t,
                        "provenance": {"source": "select-agent / Australia-Group listed toxin "
                                                 "(42 CFR 73, 7 CFR 331, 9 CFR 121; Australia Group control list)"},
                        "added_in": "0.1.0"})
    for o in sorted(SELECT_AGENT_TOXIN_ORGANISMS):
        entries.append({"id": "SAORG-" + o.replace(" ", "-"), "type": "select_agent_toxin_organism", "value": o,
                        "provenance": {"source": "source organism of a listed select-agent toxin; screened only "
                                                 "together with a toxin descriptor, which is how a listed toxin is "
                                                 "named indirectly"},
                        "added_in": "0.1.0"})
...
if __name__ == "__main__":
    ...
    released = sys.argv[2] if len(sys.argv) > 2 else "2026-07-21"
    out_dir = ROOT / "bio_firewall" / "vendored_data" / "hazard_kb"
