...
.PHONY: install lint test reproduce reproduce-local prereg-sha clean

PY ?= python

install:                       ## editable install + dev deps (pulls pen-stack>=6.6.0)
...
# (open data only): the WS-COMBINE-MONO monotonicity proof + the full test suite (which validates the cargo/
