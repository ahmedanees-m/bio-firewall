...
![Tests](https://img.shields.io/badge/tests-165%20passing-success.svg)
...
Or run the published container (Python 3.11 pinned, no host setup, nothing to build). The image is self-contained:
the screen, the signed hazard KB, and the vendored open data all ship inside it, so the demo and the committed-data
reproduction run with no external data and no network.
...
and escalation, P6 red-team, P7 audit, P8 calibration, and P9 managed access. With built-in screening (P2),
...
|   |-- access/managed.py             P9 managed access: tiered access by verdict x user legitimacy; screen_managed()
|   |-- respond/graded.py             allow/partial/flag/refuse taxonomy + partial content gate
|   |-- standards/                    nist_export.py (NIST-compatible benchmark) + ibbis.py (DSSC + OSTP)
|   |-- adapters/cloudlab_gate.py     gate the PEN-STACK cloud-lab bridge on a verified allow passport
|   |-- adapters/writespec.py         screen the typed PEN-STACK WriteRequest (SBOL3) directly
|   |-- adapters/reconcile.py         conservative-meet reconcile with pen-stack's in-design safety_gate verdict
...
|   |-- kb/                           versioned, signed hazard knowledge base loader
...
|       |   |-- conformal_bench.py            B4b: false-refuse certificate + monotone confidence
|       |   |-- decomp_redteam.py             B5: decomposition red-team
|       |   |-- locus_outcome.py              B6: locus outcome floor on VISDB
|       |   |-- edit_mech_bench.py            B8: de-novo fusion generalization
|       |   |-- locus_pos_bench.py            B9: positional locus coverage
|       |   |-- conformal_np_bench.py         NP-conformal vs scalar head-to-head (documented null)
|       |   |-- nvidia_headtohead.py          the control-vs-advisor panel (A/B/C/D via NVIDIA NIM)
|       |-- cargo_bench/run.py, decorr.py, struct_bench.py, struct_gated_bench.py   B2 gate; B2b; B10; gated struct
|       |-- headtohead/              the control-vs-advisor experiments (fabrication / paraphrase / jailbreak)
|-- locus_mouse_outcome_validation.py   CCGD outcome-validation runner (non-circular)
|-- bio_firewall/vendored_data/      open (CC0 / CC-BY) hazard data as parquet / yaml; hazard_kb/ signed KB releases
|-- data/locus_outcome_inputs/       CCGD-derived human-ortholog driver lists (ccgd_recurrent/all.txt) + SOURCE
|-- results/locus_mouse_outcome*.json   frozen CCGD outcome-validation results (held-out AUROC 0.605, OR 3.34)
...
|-- prereg/                          ws_biofirewall.yaml + ws_locus_mouse_outcome.yaml + ws_cloudlab_gate/writespec/verify_reconcile.yaml
|-- tests/                           167 tests (165 pass on a pip install; incl. the data-license CI gate, Tier-1 100%-catch regression gate, and 21 adversarial cargo-screen regression tests)
|-- Makefile, REPRODUCTION.md        one-command reproduction + the clean-image protocol
|-- Dockerfile, .dockerignore        pinned-Python reproduction image (build-time smoke test on screen() + verify_kb)
|-- .github/workflows/               ci.yml (lint + the full suite against the pinned pen-stack);
|                                    release-image.yml (publishes the verified image to ghcr.io on a release)
...
(`deepseek-v4`, `llama-4-maverick`, `qwen3-next-80b`), each given its best configuration. At the powered sample sizes, two of the four dimensions (C and D)
separate the firewall from the LLMs and two (A and B) do not. Full detail: [docs/HEADTOHEAD.md](docs/HEADTOHEAD.md) and
...
A, B, and D are the frozen 2026-06-17 measurement. The cargo screen (C) was re-run on 2026-07-19 across five reachable
models, because the earlier per-model cargo result was not committed and the models had shifted; the committed
aggregate (counts, rates, bootstrap CIs) is `results/nvidia_headtohead/cargo_llm_seqonly_rerun.json`.

| C: cargo screen, seq-only (re-run 2026-07-19) | TPR [95% CI] | FPR | Refused to engage |
|---|:--:|:--:|:--:|
...
(Llama-4-Maverick was unreachable at evaluation time and is excluded.)

- **No tested LLM screens a cargo sequence.** Across five reachable models none reaches the ESM classifier's 0.72 at
  1% FPR: three allow almost every toxin (TPR 0.01-0.05), DeepSeek flags most but at a prohibitive 18% false-positive
  rate, and the two frontier models refuse 45-47% of sequences. Behaviour shifted sharply from the June run, so it is
  uncalibrated and version-unstable. This is the role of the cargo ML gate.
...
### Cargo decorrelation and the certified false-refuse bound
...
- **Cargo decorrelation (B2b).** Is the cargo signal function-driven or the amino-acid-composition shortcut?
  ...
  ranking.
- **Conformal false-refuse ceiling (B4b).** A distribution-free guarantee an LLM
  ...
  earlier confidence inversion: monotone high 1.00 > moderate 0.69 > low 0.10, with out-of-knowledge-base allows routed to low
  confidence (9 of 10 of which are genuine misses).

### Cross-call decomposition defence
...
- **Monotone combiner (B7), pass.** The max-severity cascade is replaced by a noisy-OR combiner that is provably
  ...
  where a max is flat), and hard-rule-exact, with decisions identical to the max-severity cascade it replaces.
...
- **Structural channel (B10), negative result at the operating point.** A composition-free fold signal (AlphaFold-DB
  ...
  composition-free it independently corroborates the non-compositionality result at the ranking level. The
...
- **Living, signed hazard KB** ([docs/HAZARD_KB.md](docs/HAZARD_KB.md)): 114 versioned, provenanced, HMAC-signed
...
This work completes the artifact against the field, carrying in three reference corrections including the ABC-Bench
venue (NeurIPS 2025 BioSafe GenAI workshop, not ICML). Per a pre-registered publication gate, three workstreams land and two are gated
...
- **Neyman-Pearson conformal selection, documented null.** On the firewall corpus (COSMIC v104, gene-disjoint split,
  ...
  (0.05 -> 0.017, 0.10 -> 0.064), which the discrete scalar threshold cannot do (it sits at a fixed
  ...
  not met. NP's value is calibrated control, not more power; the certified false-refuse bound stands as the
...
- **Confidence-gated structural fusion, documented null with a sharper diagnosis.** Gating the fold channel on mean
  ...
  so gating fuses almost everywhere and the failure is fold-distance on the <=40%-identity split, not low
...
### Locus outcome-validation

This work converts the project's central standing limitation: the locus axis previously flagged on mechanism and was
not outcome-validated. The screening code is unchanged; this work adds an outcome-validation against real in vivo
insertional-oncogenesis drivers from mouse transposon forward-genetic screens (CCGD, the Candidate Cancer Gene
Database; Abbott et al., 2015). The positive sets were re-derived from the live source and the result re-run to an
exact match before integration.
...
- **It reconciles the VISDB null.** The same axis was anti-predictive on the open human catalogue (VISDB, AUROC 0.449),
...
- The locus axis routes elevated risk to human review and does not output a cancer probability; it flags on mechanism
  rather than emitting a calibrated rate. It is outcome-validated against mouse in vivo
...
- The two gated strengtheners are documented nulls, not shipped claims. Neyman-Pearson conformal selection adds
  calibrated control but not power at matched alpha, so the certified bound stands; confidence-gated structural
  fusion does not lift the 1%-FPR operating point, so the structure channel stays a ranking-level corroborator. A
  full-trajectory monitor and a scaled, categorized red-team are an explicit fast-follow.
